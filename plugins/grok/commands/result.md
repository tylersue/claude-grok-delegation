...
- Fallback 1 (the documented >255-byte case): if that group dir does not exist, scan the group dirs under `sessions/` for one containing a `.cwd` file whose content equals the absolute repo path (slug+hash naming). This read happens during the locate step, before the directory-level containment check below runs at all — so each candidate `.cwd` path is itself canonicalized (realpath, resolving symlinks) and prefix-verified inside the canonicalized `${GROK_HOME:-$HOME/.grok}/sessions/` root immediately before it is opened, using the same `python3 -c` one-liner idiom as the containment check below, retargeted to that `.cwd` leaf path. On ESCAPED, refuse and stop — read NOTHING, name the violation plainly ("session file resolves outside the sessions tree"), and point the user at `grok -r <session-id>` as the manual fallback; this is NOT a degrade case, exactly like the directory-level check below.
...
Containment check (before ANY read — runs PER FILE (leaf), every time a file under the session dir above is opened, not only once at the directory level: the directory itself, the Fallback-1 `.cwd` candidate above, the summary.json metadata read below, and the updates.jsonl on-disk transcript read below each get their own canonicalize-and-verify pass immediately before that specific file is opened):
- Canonicalize the located session dir (resolve symlinks) and verify it resolves inside the canonicalized `${GROK_HOME:-$HOME/.grok}/sessions/` root: require the resolved candidate to equal the resolved root, or to start with the resolved root plus a path separator (the trailing-separator guard prevents a `sessions-evil` vs `sessions` prefix collision). Use the same `python3 -c` one-liner idiom as the URL-encoding step above, and `os.environ['HOME']` (never `os.path.expanduser('~')`) for the GROK_HOME/HOME fallback:
  ...
  " "$SESSION_DIR"
  ```
- The identical gate applies per leaf file, retargeting `sys.argv[1]` to the leaf path immediately before it is opened — not only to the session directory: `$SESSION_DIR/summary.json` before the metadata read, `$SESSION_DIR/updates.jsonl` before the on-disk transcript read, and each candidate `.cwd` path before the Fallback-1 locate-step read. The directory itself resolving CONTAINED does not exempt the leaves inside it — a symlinked leaf (e.g. `summary.json -> ${GROK_HOME}/auth.json`) still resolves outside the sessions root and must be caught at its own gate, immediately before that file is opened.
- On ESCAPED (e.g. a symlinked group or session directory pointing outside the sessions tree, or a symlinked leaf file such as `summary.json`, `updates.jsonl`, or a `.cwd` candidate): refuse and stop — read NOTHING. Name the violation plainly ("session directory resolves outside the sessions tree" at the directory level, or "session file resolves outside the sessions tree" at a per-file leaf level) and point the user at `grok -r <session-id>` as the manual fallback. A failed containment check — directory-level or per-file (leaf) — is NOT a degrade case — it is the confidentiality boundary itself, so it never silently continues.

Metadata (always read, regardless of which retrieval branch below fires):
- From `summary.json` (plain JSON — the Read tool or a python3 one-liner): `$SESSION_DIR/summary.json` is itself canonicalized (realpath, resolving symlinks) and prefix-verified inside the canonicalized `${GROK_HOME:-$HOME/.grok}/sessions/` root, using the containment idiom above retargeted to this leaf, immediately before it is opened — on ESCAPED, refuse and report (read NOTHING) exactly as the directory-level check above does; this is NOT a degrade case. Once contained: generated title and session summary, created/updated timestamps, model id, message counts, and agent name / parent session id when present. `grok export` output carries NO session-level metadata, so this read is not gated by the PRIMARY/FALLBACK branch below — it always runs.
...
- FALLBACK (triggered on ANY export failure — grok binary missing, an unknown/missing `export` subcommand, a nonzero exit, or empty/unusable output; trigger on the failure itself, not a specific error-text match): fall back to `updates.jsonl` in the session dir located above — the final assistant message(s). `$SESSION_DIR/updates.jsonl` is itself canonicalized (realpath, resolving symlinks) and prefix-verified inside the canonicalized `${GROK_HOME:-$HOME/.grok}/sessions/` root, using the containment idiom above retargeted to this leaf, immediately before it is opened — on ESCAPED, refuse and report (read NOTHING) exactly as the directory-level check above does; this is NOT a degrade case. Once contained, the file can be large — read only its TAIL (e.g. `tail -n 200 updates.jsonl`), then extract the text of the last agent/assistant message event(s); each line is a self-contained JSON event. Never load the whole file.
