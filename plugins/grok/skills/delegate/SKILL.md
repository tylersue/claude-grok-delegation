...
- After relaying grok's answer, emit a `Sandbox:` line stating the confinement state: `Sandbox: strict` when the sandboxed Attempt 1 succeeded, or `Sandbox: UNAVAILABLE — reads unconfined (<reason>)` when the retry above fired — derived from whether Attempt 1 succeeded or the retry degraded it, never re-derived from grok's own output prose. Always emit this line; its absence is itself a defect.
