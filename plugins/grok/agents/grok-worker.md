...
- **Follow-up cleanup rule (D-07, belt-and-braces for SIGKILL):** the trap catches normal exit and SIGTERM, but nothing catches SIGKILL. After any timeout or error, your next Bash call — before you report — removes both temp files: `rm -f "$PROMPT_FILE" "$BASELINE"`.
